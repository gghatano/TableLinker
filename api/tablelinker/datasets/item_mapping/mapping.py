        ...
        for j in range(1, size2 + 1):
            ...
            for i in range(1, size1 + 1):
                ...
                if list1[i - 1] == list2[j - 1]:
                    ...
                    subcost = _subcost
                    if flag == "word":
                        simval = strsim(list1[i - 1], list2[j - 1])
                        subcost = (1 - simval) * 2
...
class Similarity(object):
    ...
    def __init__(self, dir_path=None):
        ...
        指定されたディレクトリまたは環境変数が指すディレクトリ、
        またはいずれも未指定の場合は ``./transformer/{tokenizer, model}`` に
        保存し、再利用する。
        """
        dir_path = dir_path or os.environ.get('TRANSFORMER_DIR') or \
            os.path.join(os.path.dirname(__name__), "transformer")

        tokenizer_dir = os.path.join(dir_path, "tokenizer")
        if os.path.exists(os.path.join(tokenizer_dir, "vocab.txt")):
            logger.debug("Loading saved tokenizer from {}".format(tokenizer_dir))
            self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_dir)
        else:
            logger.info("Loading tokenizer from web")
            ...
            logger.info("Save tokenizer to {}".format(tokenizer_dir))

        model_dir = os.path.join(dir_path, "model")
        if os.path.exists(os.path.join(model_dir, "config.json")):
            logger.info("Loading saved model from {}".format(model_dir))
            self.model = AutoModel.from_pretrained(model_dir)
        else:
            logger.info("Loading model from web")
            os.makedirs(model_dir, 0o755, exist_ok=True)
            self.model = AutoModel.from_pretrained("cl-tohoku/bert-base-japanese-whole-word-masking")
            self.model.save_pretrained(model_dir)
            logger.info("Save model to {}".format(model_dir))
    ...
    def get_vectors(self):
        ...
        事前計算済みベクトルをメモリに展開する
        """
        token_embeddings = self.model.get_input_embeddings().weight.clone()
...
class ItemsPair(object):
    ...
    def __init__(self, items0: List[str], items1: List[str], similarity=None):
        """
        Parameters
        ----------
        items0: List[str]
        ...
        self.mxsim = None
        self.mxed = None
        self.similarity = similarity or Similarity.get_instance()
    ...
    def mapping(self):
        ...
        for pair in pairs:
            ...
            results.append(
                [
                    i if i < len(self.items0) else None,
                    self.items0[i] if i < len(self.items0) else None,
                    j if j < len(self.items1) else None,
